...
<div class="card">
    <div class="card-body">
        <form action="{{ route('admin.jenis-cuti.update', $jenisCuti->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('kode_cuti') is-invalid @enderror" 
                       id="kode_cuti" name="kode_cuti" value="{{ old('kode_cuti', $jenisCuti->kode_cuti) }}" required>
                @error('kode_cuti')
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('nama_cuti') is-invalid @enderror" 
                       id="nama_cuti" name="nama_cuti" value="{{ old('nama_cuti', $jenisCuti->nama_cuti) }}" required>
                @error('nama_cuti')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('deskripsi') is-invalid @enderror" 
                          id="deskripsi" name="deskripsi" rows="4">{{ old('deskripsi', $jenisCuti->deskripsi) }}</textarea>
                @error('deskripsi')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('maksimal_hari') is-invalid @enderror" 
                               id="maksimal_hari" name="maksimal_hari" 
                               value="{{ old('maksimal_hari', $jenisCuti->maksimal_hari) }}" min="0">
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('is_dibayar') is-invalid @enderror" 
                            ...
                            <option value="1" {{ old('is_dibayar', $jenisCuti->is_dibayar) == '1' ? 'selected' : '' }}>Dibayar</option>
                            <option value="0" {{ old('is_dibayar', $jenisCuti->is_dibayar) == '0' ? 'selected' : '' }}>Tidak Dibayar</option>
                        </select>
                        @error('is_dibayar')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
